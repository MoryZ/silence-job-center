...
public class DistributedLockHandler {
    ...
    public void lockWithDisposableAndRetry(LockExecutor lockExecutor,
        ...
        Retryer<Boolean> retryer = RetryerBuilder.<Boolean>newBuilder()
                ...
                .withRetryListener(new RetryListener() {
                    ...
                    public <V> void onRetry(final Attempt<V> attempt) {
                        ...
                        SilenceJobLog.LOCAL.debug("第【{}】次尝试获取锁. lockName:[{}] result:[{}] treadName:[{}]",
        ...
        try {
            ...
            SilenceJobLog.LOCAL.warn("lock execute error. lockName:[{}]", lockName, throwable);
        } finally {
            if (lock) {
                lockProvider.unlock();
                SilenceJobLog.LOCAL.debug("[{}] 锁已释放", lockName);
            } else {
                // 未获取到锁直接清除线程中存储的锁信息
                LockManager.clear();
            }
    ...
    public void lockWithDisposable(LockExecutor lockExecutor, String lockName, Duration lockAtMost) {
        ...
        try {
            ...
            SilenceJobLog.LOCAL.error("lock execute error. lockName:[{}]", lockName, e);
        } finally {
            if (lock) {
                lockProvider.unlock();
                SilenceJobLog.LOCAL.debug("[{}] 锁已释放", lockName);
