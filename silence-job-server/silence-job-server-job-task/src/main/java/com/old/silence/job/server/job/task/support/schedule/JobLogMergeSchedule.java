...
import com.old.silence.platform.job.server.common.util.PartitionTaskUtils;
import com.old.silence.platform.job.server.domain.model.JobLogMessage;
import com.old.silence.platform.job.server.domain.model.JobTaskBatch;
import com.old.silence.platform.job.server.infrastructure.persistence.dao.JobLogMessageDao;
import com.old.silence.platform.job.server.infrastructure.persistence.dao.JobTaskBatchDao;
import com.old.silence.platform.job.server.job.task.dto.JobPartitionTaskDTO;
...
public class JobLogMergeSchedule extends AbstractSchedule implements Lifecycle {
    ...
    public void processJobLogPartitionTasks(List<? extends PartitionTask> partitionTasks) {
        ...
        for (Map.Entry<BigInteger/*taskId*/, List<JobLogMessage>> jobLogMessageMap : jobLogMessageGroupList) {
            List<BigInteger> jobLogMessageDeleteBatchIds = new ArrayList<>();
            ...
            List<String> mergeMessages = jobLogMessageMap.getValue().stream().map(k -> {
                        ...
                        return JSON.parseObject(k.getMessage(), List.class);
                    })
                    .reduce((a, b) -> {
                        // 合并日志信息
