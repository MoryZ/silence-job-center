...
import com.old.silence.platform.job.server.common.rpc.client.SilenceJobRetryListener;
...
import com.old.silence.platform.job.server.retry.task.support.RetryTaskLogConverter;
...
public class RequestCallbackClientActor extends AbstractActor {
    ...
    public RequestCallbackClientActor(RetryTaskDao retryTaskDao, RetryDao retryDao) {
    ...
    private void doCallback(RequestCallbackExecutorDTO executorDTO) {
        ...
        if (Objects.isNull(retry)) {
            JobLogMetaDTO jobLogMetaDTO = RetryTaskConverter.INSTANCE.toJobLogDTO(executorDTO);
            jobLogMetaDTO.setTimestamp(nowMilli);
            SilenceJobLog.REMOTE.error("retryTaskId:[{}] 任务调度失败. 失败原因: 重试任务不存在 <|>{}<|>", executorDTO.getRetryTaskId(),
                    jobLogMetaDTO);
            return;
        }
        ...
        try {
            ...
            ApiResult<Boolean> dispatch = rpcClient.callback(retryCallbackRequest);
