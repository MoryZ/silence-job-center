...
import com.old.silence.job.common.network.SilenceJobNetworkProperties;
import com.old.silence.job.log.SilenceJobLog;

/**
 * @author moryzang
 */
public class SilenceJobNetworkUtils {


    private static final Pattern IPV4_PATTERN = Pattern.compile(
            "^(([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.){3}([01]?\\d\\d?|2[0-4]\\d|25[0-5])$");

    private final SilenceJobNetworkProperties properties;

    public SilenceJobNetworkUtils(SilenceJobNetworkProperties properties) {
    ...
    public InetAddress findFirstNonLoopbackAddress() {
        ...
        try {
            ...
            for (Enumeration<NetworkInterface> nics = NetworkInterface.getNetworkInterfaces(); nics.hasMoreElements(); ) {
                ...
                if (ifc.isUp()) {
                    SilenceJobLog.LOCAL.trace("Testing interface: " + ifc.getDisplayName());
                    ...
                    if (!ignoreInterface(ifc.getDisplayName())) {
                        for (Enumeration<InetAddress> addrs = ifc.getInetAddresses(); addrs.hasMoreElements(); ) {
                            ...
                            if (isValidAddress(address) && isPreferredAddress(address)) {
                                SilenceJobLog.LOCAL.trace("Found non-loopback interface: " + ifc.getDisplayName());
                                result = address;
                            }
                        }
                    }
            ...
            SilenceJobLog.LOCAL.error("Cannot get first non-loopback address", ex);
        }
    ...
    private boolean isPreferredAddress(InetAddress address) {
        ...
        SilenceJobLog.LOCAL.info("Ignoring address: " + address.getHostAddress());
        return false;
    }
    ...
    boolean ignoreInterface(String interfaceName) {
        for (String regex : this.properties.getIgnoredInterfaces()) {
            if (interfaceName.matches(regex)) {
                SilenceJobLog.LOCAL.trace("Ignoring interface: " + interfaceName);
                return true;
            }
        }
        return false;
